...
STRATEGY:
1. run_pipeline to see errors
2. inspect suspicious stages
3. fix bugs earliest stage first
...
def build_prompt(obs) -> str:
    ...
    history_text = "\n".join(obs.actions_taken[-4:]) if obs.actions_taken else "No actions yet."
...
def run_task(client: OpenAI, task_name: str) -> Dict[str, Any]:
    ...
    while not done and steps_used < max_steps:
        ...
        try:
            ...
            print(f"[STEP] step={steps_used+1} action=run_pipeline reward=0.000 score=0.000 error={exc}", flush=True)
        ...
        print(
            f"[STEP] step={steps_used} action={action_str} "
            f"reward={reward.value:.3f} score={obs.current_score:.3f}",
            flush=True
        )
    ...
    if not done:
        ...
        if "grader_breakdown" in info:
            ...
            final_score      = grader_breakdown.get("final_score", 0.0)
        print(
            f"[STEP] step={steps_used} action=submit "
            f"reward={reward.value:.3f} score={final_score:.3f}",
            flush=True
        )

    # ── [END] block ────────────────────────────────────────
    print(f"[END] task={task_name} score={final_score:.4f} steps={steps_used}", flush=True)
...
def main() -> None:
    ...
    print(f"Pipeline Debugger — Baseline Inference", flush=True)
    ...
    for task_name in TASKS:
        ...
        if elapsed > 18 * 60:
            print(f"WARNING: Time limit approaching ({elapsed/60:.1f}min). Stopping.", flush=True)
            break
    ...
    for r in results:
        print(f"  {r['task']:<10} score={r['final_score']:.4f} steps={r['steps_used']}", flush=True)
